...
if __name__ == "__main__":
    ...
    D = C.map(lambda line: (line[0].encode("ascii","ignore"), line[1].lower()))
  
    E = D.map(lambda line: (line[0], re.sub("\W", " ", line[1])))
    F = D.map(lambda line: (line[0], re.sub("\\W+"," ", line[1])))
    G=F.map(lambda line: line[0] + "\t" + removeWord(str(line[1])))
